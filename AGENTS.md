...
- 애매한 인프라 선택은 임의로 확정하지 않고 해당 이슈의 `design.md` 또는 `plan.md`에 결정 필요 항목으로 남긴다.

## 문서와 기록

- 작업 문서가 필요하면 `docs/tasks/{ISSUE_NUMBER}/` 아래에 둔다.
- 요구사항이 모호하거나 설계 결정이 필요한 작업은 `design.md`에 승인된 설계를 기록한다.
- 구현이 여러 단계이거나 인수인계용 계획이 필요한 작업은 `plan.md`에 구현 순서, 발견 사항, 설계 변경, 검증 결과를 기록한다.
- 승인된 `design.md` 또는 `plan.md`를 단일 기준 문서로 사용하고 같은 내용을 별도 작업 문서에 중복하지 않는다.
- 단순하거나 범위가 명확한 작업은 별도 작업 문서를 만들지 않는다.
- 기존 `checklist.md`와 `context-notes.md`는 과거 기록으로만 유지하고 새 작업 기록을 추가하지 않는다.
- 이슈별 작업 기록은 해당 이슈 디렉터리 안에서만 갱신한다.
...
- 가능하면 커밋 1개는 변경 30줄 내외로 한다. 논리 단위로 작게 커밋하고, PR은 리뷰 가능한 크기로 유지한다.
