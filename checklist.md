...
- [x] 현재 IaC Wiki와 BE·AI Wiki 게시본의 구조와 운영 문서 흐름을 비교했다.
- [x] production 장애 대응 중심의 역할 기반 전면 개편안을 확정했다.
- [x] 확정 설계 문서를 검토하고 구현 계획을 작성한다.
...
## 2026-07-22 LAN-192 Grafana 장애 알림 개선
...
## 2026-08-25 LAN-372 개발 환경 Sentry 오류 대응

- [x] 개발 EC2가 shared 콘텐츠 bucket과 CloudFront 출력을 소비하도록 user-data를 확장한다.
- [x] 개발 EC2 role에 `content/inbox/*` 업로드 권한을 최소 범위로 추가한다.
- [x] user-data 렌더링·콘텐츠 업로드 IaC 계약 테스트를 통과한다.
- [x] `terraform fmt -recursive`를 실행한다.
- [x] 최초 콘텐츠 설정의 개발·운영 Terraform apply와 IAM 최소 권한을 live에서 확인한다.
- [x] 기존 EC2에도 반영되도록 SSM 배포 전 `runtime-env` 동기화 경로를 추가한다.
- [x] EC2 생성과 SSM 배포가 같은 runtime 템플릿을 사용하는 계약 테스트를 통과한다.
- [x] runtime 동기화 변경의 개발·운영 Terraform plan을 확인한다.
...
- [ ] 만료된 PROFILE을 현재 사실로 답하는 LLM 재현 오류를 보완하고 재검증한다.
- [ ] 재검증 통과 후 운영 SSM을 변경하고 BE 재배포와 실제 기억 검색을 확인한다.
