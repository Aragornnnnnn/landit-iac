...
- 사용자 승인 후 관리자 profile로 saved plan `/private/tmp/lan430-develop-golden-audio-admin.tfplan`을 적용했다. `develop-golden-audio-read` IAM inline policy만 `1 added, 0 changed, 0 destroyed`로 생성됐다.
- apply 후 `-target` 없는 전체 saved plan `/private/tmp/lan184-develop-post-apply-full.tfplan`은 `No changes`다. SSM 배포 문서, Scheduler와 나머지 develop Terraform 관리 대상에 drift가 없다.
