...
develop EC2의 Compose API도 shared Terraform state의 같은 두 값을 runtime env로 받으며, EC2 role은 `content/inbox/*`에 대한 `s3:GetObject`, `s3:PutObject`만 허용합니다. production ECS API task role도 같은 prefix의 조회와 업로드만 허용합니다.
