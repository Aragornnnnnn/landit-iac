...
실제 endpoint가 준비되기 전에는 `grafana_otlp_enabled=false`를 유지합니다. 활성화할 때 아래 입력을 함께 설정합니다.

```hcl
grafana_otlp_enabled  = true
grafana_otlp_endpoint = "https://<Grafana Cloud OTLP endpoint>/otlp"
```
...
BE는 30초 간격으로 OTLP metric export를 활성화하고 base endpoint에 `/v1/metrics`를 붙인 signal-specific endpoint를 사용합니다. AI는 base endpoint를 사용합니다. 공통 resource attribute는 `service.namespace=landit`, `deployment.environment.name=develop|prod`이고, service name은 각각 `landit-be`, `landit-ai`입니다.

## CloudWatch Logs 전달

실제 Grafana Cloud AWS Logs endpoint와 Secrets Manager ARN이 준비되기 전에는 `grafana_logs_enabled=false`를 유지합니다. 활성화할 때 아래 입력을 함께 설정합니다.

```hcl
grafana_logs_enabled    = true
grafana_logs_endpoint   = "https://<Grafana Cloud AWS Logs endpoint>/aws-logs/api/v1/push"
grafana_logs_secret_arn = "arn:aws:secretsmanager:ap-northeast-2:982529430654:secret:<secret name>"
```
