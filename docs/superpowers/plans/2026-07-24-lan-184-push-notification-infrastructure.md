...
BE는 `messageId`를 UUID가 아닌 문자열로 처리한다. `occurredAt`을 `Asia/Seoul`로 변환해 `reviewDate`를 계산한다.
...
- [ ] plan JSON에서 삭제·교체와 기존 Worker·jobs Queue·ECS Service 변경 여부를 확인한다.
- [ ] apply는 실행하지 않는다.
