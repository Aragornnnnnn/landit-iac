...
- prod Terraform root에 Python Lambda와 Function URL을 둔다.
- Function URL은 API Gateway 없이 Sentry webhook을 받는다.
- Lambda는 Sentry Internal Integration에 설정한 고정 header를 prod SSM relay token과 상수 시간 비교한다.
- 인증에 실패한 요청은 Discord로 보내지 않고 `401`로 종료한다.
- payload environment가 없거나 `prod`가 아니면 Discord로 보내지 않는다.
- Lambda는 Sentry issue 제목, project 또는 rule 이름, level, environment, issue URL을 Discord embed로 변환한다.
...
Lambda 실행 role은 두 parameter에 대한 `ssm:GetParameter`만 허용한다. secret 값은 Lambda 환경변수, Terraform variable, state, 문서와 명령 출력에 기록하지 않는다.

## 설정 관리

Grafana Cloud의 contact point와 alert rule은 UI에서 관리한다. Sentry alert rule은 Sentry Internal Integration의 alert action으로 Lambda relay를 호출한다. Lambda와 IAM, log retention, Function URL은 prod Terraform state에서 관리한다.
...
- Sentry Internal Integration에는 relay token을 고정 header로 설정한다.
- Lambda는 relay token을 상수 시간 비교하고 prod가 아닌 payload를 전달하지 않는다.
- Function URL은 public endpoint이므로 timeout과 reserved concurrency를 낮게 제한해 오용 범위를 제한한다.
- Lambda는 Discord webhook URL과 relay token을 SSM에서 복호화해 읽고 로그에 출력하지 않는다.
...
1. Lambda unit test로 인증 실패, non-prod 제외, payload 변환을 확인한다.
2. 올바른 relay token을 사용한 검증 request가 Lambda를 거쳐 `#alerts-sentry-prod`에 도착하는지 확인한다.
3. 잘못된 token과 non-prod request가 Discord로 전달되지 않는지 확인한다.
...
6. 운영 rule을 복제한 단기 test rule로 firing 메시지를 확인한다.
7. test rule 조건을 정상화해 Resolved 메시지를 확인한다.
8. 두 채널에 develop 알림이 전달되지 않는지 설정을 다시 확인한다.
9. 검증용 test rule과 test issue를 정리하고 운영 rule이 Normal 상태인지 확인한다.
...
- [AWS Lambda Function URL](https://docs.aws.amazon.com/lambda/latest/dg/urls-configuration.html)
