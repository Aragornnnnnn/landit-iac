...
- prod Terraform root의 Python Lambda와 Function URL을 유지한다.
- Sentry App webhook의 기본 HTTP timeout은 1초이므로 ingress invocation은 Discord 응답을 기다리지 않는다.
- ingress는 body와 `Sentry-Hook-Signature` 형식을 확인한 뒤 같은 Lambda를 `Event` 방식으로 비동기 호출하고 즉시 `204`를 반환한다.
- Function URL event와 내부 delivery event는 최상위 event 구조로 구분한다. 외부 request body에 내부 mode를 넣어도 delivery 경로로 진입할 수 없다.
...
- Lambda memory는 512MB, timeout은 10초, reserved concurrency는 2로 제한한다. ingress cold request가 1초 안에 응답하는지는 실제 Function URL 검증으로 확인한다.

공개 ingress는 HMAC secret을 읽기 전에 비동기 invocation을 만들기 때문에 위조 request도 delivery invocation 하나를 만들 수 있다. delivery에서 HMAC을 반드시 검증해 Discord 오발송을 막고, signature 형식 검사·body 크기 제한·reserved concurrency로 비용과 자원 오용 범위를 제한한다. cold ingress가 1초를 넘으면 완료로 처리하지 않고 API Gateway 또는 SQS 기반 수신 경로를 별도 재설계한다.
...
- Function URL은 public endpoint이므로 body·signature 형식, timeout, memory, reserved concurrency를 제한해 오용 범위를 줄인다.
