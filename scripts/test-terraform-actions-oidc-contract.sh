...
assert_contains "${bootstrap}" '"s3:PutLifecycleConfiguration"' "S3 lifecycle 변경 IAM action이 정확해야 한다."
assert_contains "${bootstrap}" '"s3:GetBucketOwnershipControls"' "shared bucket ownership 조회 권한이 필요하다."
assert_contains "${bootstrap}" '"s3:PutBucketOwnershipControls"' "shared bucket ownership 변경 권한이 필요하다."
